...
INSERT INTO `classivirtuali` VALUES ('i8k6j5h4','Ingegneria Del Software','pablopicasso@unina.it'),('n4m3b2v1','Analisi 2','martinpalumbo@unina.it'),('o2i3k4j5','Metodi Matematici','francol@unina.it'),('op3t5p4i','Analisi 1','martinpalumbo@unina.it'),('u6j5o34k','Sistemi Operativi','antonioconte@unina.it'),('v3c4n5m6','Controlli Automatici','stefanodemartino@unina.it');
...
INSERT INTO `docenti` VALUES ('antonioconte@unina.it','Antonio','Conte','Docente','kdb1!'),('francol@unina.it','Francesco','Colella','Docente','avellino2!'),('martinpalumbo@unina.it','Martin','Palumbo','Docente','avellino3#'),('pablopicasso@unina.it','Pablo','Picasso','Docente','arte5@'),('stefanodemartino@unina.it','Stefano','De Martino','Docente','emmamarrone1!');
...
CREATE TABLE `task` (
  ...
  KEY `fk_Task_ClassiVirtuali_idx` (`ClassiVirtuali_CodiceUnivoco`) /*!80000 INVISIBLE */,
...
INSERT INTO `task` VALUES ('Equazioni Differenziali','Svolgere almeno due dei tre esercizi sulle equazioni differenziali','2025-09-01',9,'n4m3b2v1','martinpalumbo@unina.it'),('Esercizi Limiti','Svolgere almeno due dei tre esercizi proposti per poter ottenere la sufficienza','2025-07-01',9,'op3t5p4i','martinpalumbo@unina.it'),('Matlab','Consegnare lo script di matlab degli esercizi riguardanti i controllori dinamici','2025-10-11',13,'','stefanodemartino@unina.it'),('Prova Fourier','Completare la prova assegnata per essere esonerati dallo svolgimento dell\'esercizio su Fourier all\'esame','2025-06-25',10,'o2i3k4j5','francol@unina.it'),('Prova Laplace','Completare la prova assegnata per essere esonerati dallo svolgimento dell\'esercizio su Laplace all\'esame','2025-06-15',10,'o2i3k4j5','francol@unina.it'),('Prova Residui','Completare la prova assegnata per essere esonerati dallo svolgimento dell\'esercizio sui residui all\'esame','2025-06-30',10,'o2i3k4j5','francol@unina.it'),('Sequence Diagram','Sviluppare il sequence diagram del caso d\'uso allegato alla prova','2025-06-17',5,'i8k6j5h4','pablopicasso@unina.it'),('Simulazione Esame','File da completare: header.h, main.c, client.c, server.c','2025-07-12',30,'u6j5o34k','antonioconte@unina.it'),('Simulazione Esame 2','Esercitazione sui thread. Da completare: header.h, main.c, client.c, server.c','2025-08-01',30,'u6j5o34k','antonioconte@unina.it');
...
-- Dump completed on 2025-06-14 11:26:45
